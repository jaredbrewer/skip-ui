...
`Tests/SkipUITests/LayoutTests.swift` adds two tests:

1. **`testIgnoresSafeAreaInfiniteMaxHeightMeasureDoesNotOverflow`** — a genuine regression test that drives the REAL `IgnoresSafeAreaLayout` measure lambda (not a reimplementation of the guard). Robolectric's real `WindowInsets.safeDrawing` is 0, so a plain render cannot reproduce the overflow; the test bypasses that two ways: (a) it injects a synthetic `SafeArea` whose safe bounds are inset 100px on the top and bottom edges (positive vertical expansion), and (b) it hosts the layout inside a `verticalScroll` `Column`, which measures its child with `maxHeight == Constraints.Infinity` — the same unbounded intrinsic pass the sheet delivers on device. If the saturation guard is removed, the lambda computes `Int.MAX_VALUE + 200` → wraps negative → `Constraints.copy(maxHeight:)` throws during measure → the composition throws out of `setContent` → the test fails. **Verified fail-at-stock in-harness**: temporarily reverting the guard flips the suite to `FAILED 1` with exactly this test throwing `java.lang.IllegalArgumentException`; with the guard, `FAILED 0`. The crash axis is maxHEIGHT. Skipped on iOS/macOS (no Kotlin runtime / no SKIP layout path).

2. **`testIgnoresSafeAreaWithinNavigationStackRendersWithoutCrash`** — public-API smoke: `.ignoresSafeArea()` content hosted in a `NavigationStack` (whose `PresentationRoot` installs a non-nil `_safeArea`) exercises the guarded path and renders without crash.

**Caveat**: the deterministic *behavioral* reproduction is still the on-device capture documented above; Robolectric's zero real insets mean a rendered A/B cannot show the overflow. Test 1 sidesteps that by injecting a synthetic non-zero `SafeArea` plus an unbounded measure pass, so it genuinely fails without the guard.
