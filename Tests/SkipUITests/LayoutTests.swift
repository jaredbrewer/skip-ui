...
final class LayoutTests: XCSnapshotTestCase {
    ...
    // MARK: - IgnoresSafeAreaLayout: Constraints.Infinity overflow (maxHeight axis)

    /// Drives the REAL `IgnoresSafeAreaLayout` measure lambda with the exact inputs that
    /// crash on device — a positive vertical safe-area expansion together with an
    /// unbounded (`Constraints.Infinity`) maxHeight — and asserts that composition
    /// completes without throwing.
    ///
    /// This is not a reimplementation of the guard: the arithmetic under test is the
    /// production `constraints.copy(maxHeight:)` line in `ComposeLayouts.swift`. If that
    /// saturation guard is removed, the measure pass computes `Int.MAX_VALUE + expansion`,
    /// which wraps to a large negative value; `Constraints.copy(maxHeight:)` then throws
    /// `IllegalArgumentException` during measure, the exception propagates out of
    /// `setContent`, and THIS TEST FAILS. In other words the test fails if production is
    /// reverted, which the previous arithmetic-only test did not.
    ///
    /// Two things must be arranged, because Robolectric's real `WindowInsets.safeDrawing`
    /// is 0 (a plain render therefore has zero expansion and cannot reproduce the crash —
    /// this is the honest limitation of the transpiled harness):
    ///   1. A synthetic `SafeArea` is injected whose safe bounds are inset 100 px from the
    ///      presentation bounds on the top and bottom edges, forcing a positive vertical
    ///      expansion in `IgnoresSafeAreaLayout`.
    ///   2. The layout is hosted inside a `verticalScroll` `Column`, which measures its
    ///      child with `maxHeight == Constraints.Infinity` — the same unbounded height the
    ///      sheet intrinsic-measurement pass delivers on device.
    ///
    /// The crash is on the maxHEIGHT axis (unbounded vertical intrinsic measurement); the
    /// width axis is bounded in the reproduction and is not the failing axis.
    func testIgnoresSafeAreaInfiniteMaxHeightMeasureDoesNotOverflow() throws {
        #if !SKIP
        throw XCTSkip("IgnoresSafeAreaLayout measure path is Android/SKIP-only; no iOS analog")
        #else
        // Safe bounds inset 100 px top and bottom → expansionTop = expansionBottom = 100.
        // A typed Edge.Set local gives Skip the type context it needs for the set literal.
        let verticalEdges: Edge.Set = [.top, .bottom]
        let presentation = Rect(left: Float(0.0), top: Float(0.0), right: Float(1000.0), bottom: Float(2000.0))
        let safe = Rect(left: Float(0.0), top: Float(100.0), right: Float(1000.0), bottom: Float(1900.0))
        let synthSafeArea = SafeArea(presentation: presentation, safe: safe, absoluteSystemBars: verticalEdges)
        composeRule.setContent {
            EnvironmentValues.shared.setValues {
                $0.set_safeArea(synthSafeArea)
                return ComposeResult.ok
            } in: {
                // verticalScroll measures its content child with maxHeight == Constraints.Infinity.
                Column(modifier: Modifier.verticalScroll(rememberScrollState())) {
                    IgnoresSafeAreaLayout(expandInto: verticalEdges, checkEdges: verticalEdges) { _, _ in
    ...
    /// Public-API smoke: `.ignoresSafeArea()` content hosted in a `NavigationStack`
    /// (whose `PresentationRoot` installs a non-nil `_safeArea`) exercises the real
    /// `IgnoresSafeAreaLayout` measure path and renders without crashing. Under
    /// Robolectric the expansion is 0, so this cannot reproduce the overflow (see the
    /// regression test above); it guards that the guarded path composes cleanly.
    func testIgnoresSafeAreaWithinNavigationStackRendersWithoutCrash() throws {
        _ = try render(view:
            NavigationStack {
                ZStack {
